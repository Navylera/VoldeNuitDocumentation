...
class SvgFormatter(Formatter):
    ...
    positions containing ``<tspan>`` elements with the individual token styles.
    ...
    `yoffset`
        ...
        in pixels, or ``20`` else.  (This is necessary since text positions
        refer to the text baseline, not the top edge.)

    `ystep`
        Offset to add to the Y position for each subsequent line.  This should
